...
if __name__ == "__main__":
    ...
    from nutsbear.demos import attention, transformer
...
from nutsbear.demos.transformer import (
    Transformer,
    TransformerDecoder,
...
def test_TransformerEncoderLayer(torch_fkwargs):
    ...
    torch.randn(bsz, tlen, 8, **torch_fkwargs)
    torch.randn(bsz, mlen, 8, **torch_fkwargs)

    nntel = nn.TransformerEncoderLayer(
        8, 1, 16, 0, batch_first=True, **torch_fkwargs
    )
    ...
    src_key_padding_mask = torch.randint(0, 2, (bsz, slen), device=device).to(
        torch.bool
    )
    ...
    nn_src_mask = nn.Transformer.generate_square_subsequent_mask(slen).to(
        **torch_fkwargs
    )
...
def test_TransformerEncoderLayer_with_SimpleMHA(torch_fkwargs):
    ...
    tel = TransformerEncoderLayer(
        8, 1, 16, 0, attn_style="qwk", **torch_fkwargs
    )
...
def test_TransformerEncoder(torch_fkwargs):
    ...
    nntel = nn.TransformerEncoderLayer(
        8, 2, 16, 0, batch_first=True, **torch_fkwargs
    )
...
def test_TransformerDecoderLayer(torch_fkwargs):
    device, _dtype = torch_fkwargs["device"], torch_fkwargs["dtype"]
    bsz, slen, tlen, mlen = 3, 4, 6, 4
    torch.randn(bsz, slen, 8, **torch_fkwargs)
    tgt = torch.randn(bsz, tlen, 8, **torch_fkwargs)
    mem = torch.randn(bsz, mlen, 8, **torch_fkwargs)

    nntdl = nn.TransformerDecoderLayer(
        8, 2, 16, 0.0, batch_first=True, **torch_fkwargs
    )
    ...
    tgt_key_padding_mask = torch.randint(0, 2, (bsz, tlen), device=device).to(
        torch.bool
    )
    nnret = nntdl(
        tgt,
    ...
    nn_tgt_mask = nn.Transformer.generate_square_subsequent_mask(tlen).to(
        **torch_fkwargs
    )
...
def test_TransformerDecoderLayer_with_SimpleMHA(torch_fkwargs):
    ...
    tdl = TransformerDecoderLayer(
        8, 2, 16, 0.0, attn_style="qwk", **torch_fkwargs
    )
    ...
    # Forward with self-attention-mask only.
    tgt_mask = torch.randint(0, 2, (tlen, tlen), device=device).to(torch.bool)
    ret = tdl(tgt, mem, tgt_mask=tgt_mask, tgt_is_causal=False)
    assert ret.size() == (bsz, tlen, 8)
...
def test_TransformerDecoder_with_SimpleMHA(torch_fkwargs):
    ...
    ret = td(tgt, mem, tgt_mask=tgt_mask, tgt_is_causal=False)
    assert ret.size() == (bsz, tlen, 8)
...
def test_Transformer(torch_fkwargs):
    ...
    nntf = nn.Transformer(
        8, 2, 6, 6, 16, 0.0, batch_first=True, **torch_fkwargs
    )
