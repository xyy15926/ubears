...
from statbear.panel.freqs import (
    cal_entropy,
    cal_gini,
    chi2_only,
    chi_pairwisely,
...
def test_cal_entropy():
    ...
    assert cal_entropy(freqs, axis=None).shape == ()
    assert cal_entropy(freqs[0]).shape == ()
...
def test_gini():
    ...
    assert cal_gini(freqs, axis=None).shape == ()
    assert cal_gini(freqs[0]).shape == ()
...
def test_enhanced_freqs_1D():
    rows = np.concatenate([np.ones(5), np.ones(6) * 2, np.ones(7) * 3])
    ...
    ret, unisr = enhanced_freqs(
        rows, others=others, agg=lambda x: np.sum(x, axis=0)
    )
    assert np.all(ret == [[15, 5], [14, 12], [9, 21]])
    assert np.all(unisr == [1, 2, 3])
...
def test_chi2_only():
    ...
    (_ux, _uy), freqs = contingency.crosstab(X[:, 0], y)
