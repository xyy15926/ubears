...
def mon_itvl(x: Sequence, y: Sequence):
    ...
    sub = x - y
    ret = np.select(
        [~np.isnat(sub), np.isnat(sub)], [sub.astype(np.float32), np.nan]
    )
    return ret
...
# %%
def cb_fst(x: Sequence | Any, y: Sequence | Any):
...
def cb_max(x: Sequence | int | float, y: Sequence | int | float):
...
def cb_min(x: Sequence | int | float, y: Sequence | int | float):
...
# %%
def isin(x: Sequence | Any, y: Sequence | Container):
    """Check if `x` in `y`.
